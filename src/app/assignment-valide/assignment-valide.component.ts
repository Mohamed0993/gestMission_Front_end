...
@Component({
  selector: 'app-assignment-valide',
  templateUrl: './assignment-valide.component.html',
  styleUrls: ['./assignment-valide.component.css']
})
export class AssignmentValideComponent implements OnInit, AfterViewInit {

  model: NgbDateStruct;

  public assignmentL !: Observable<Array<Assignment>>
  ...
  getAll(){
    this.assignmentL = this.assignment.getAll().pipe(
      catchError(err => {
  ...
  /*getDataAssign():[Assignment]{
    let tmp:Array<Assignment> = []
    this.assignment.getAll().subscribe(data => {
  ...
  printOrder(assign:Assignment){

    this.decompteService.printOrder(assign.assignmentId).subscribe({
      next:(data)=>{
        console.log(data);
      }
    })
  ...
  rejectAssign(assignmenValid: Assignment) {
    this.assignment.rejectAssignment(assignmenValid).subscribe({
      next: (data)=>{
